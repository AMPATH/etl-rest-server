...
function generateRemindersReport(
  ...
  return new Promise((resolve, reject) => {
    ...
    prepReminder
      .isPatientEnrolledInPrep(combineRequestParams)
      .then((b) => {
        if (b) {
          isPrepPatient = true;
          reportParams = etlHelpers.getReportParams(
              ...
              if (isPrepPatient) {
                if (results.results.results.length > 0) {
                  let processedResults = prepReminder.generateReminders(
                    ...
                    results.results.resultseidReminders
                  );
                  results.result = {
