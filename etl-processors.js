  ...
  function cdmMedicationChange (results) {
    ...
    _.each(med_results, function (row, index) {
      var row_promise = new Promise(function (resolve, reject) {
        var current  = row.dm_meds_name ? row.dm_meds_name.split(',') : null;
        var previous = '';
        if (index > 0) {
          var previousRow = med_results[index - 1];
          previous        = previousRow.dm_meds_name ? previousRow.dm_meds_name.split(',') : null;
        ...
        if (!arraysEqual(current, previous)) {
          if (row.prescriptions) {
            ...
            resolve({prescriptions: null, encounter_datetime: row.encounter_datetime});
          }
      ...
      concept_name_dao.getConceptNames().then(function (concepts) {
        _.each(prescriptions, function (row) {
          ...
          _.each(prescription_details, function (details) {
            ...
            result['htn_med']               = dataRow.htn_med;
            
            if (hasStatus) {
              var status                = details.split('=')[1].replace('!!', '').trim();
              result['medication_plan'] = helpers.getConceptNames(concepts, status) ? helpers.getConceptNames(status) : status;
