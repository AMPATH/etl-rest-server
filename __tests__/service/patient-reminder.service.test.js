    ...
    expect(
      ...
      expect.arrayContaining([
        expect.objectContaining({
          display: { banner: true, toast: true },
          message: expect.stringContaining(
            'Patient requires viral load. A pregnant or breastfeeding patient with vl <= 400 requires a viral load test every 6 months.'
          ),
          title: 'Viral Load Reminder',
          type: 'danger'
        })
    ...
    expect(
      ...
      expect.arrayContaining([
        expect.objectContaining({
          display: { banner: true, toast: true },
          message: expect.stringContaining(
            'Viral load test that was ordered on: (13-06-2021) resulted in an error. Please reorder.'
          ),
