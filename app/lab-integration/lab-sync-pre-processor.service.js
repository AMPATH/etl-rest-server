...
function getPersonAttributes(patientUuid) {
  var uri = getRestResource('/' + config.openmrs.applicationName + '/ws/rest/v1/person/' + patientUuid);
  var queryString = {
...
function getPatientVisits(patientUuid) {
  var uri = getRestResource('/' + config.openmrs.applicationName + '/ws/rest/v1/visit');
  var queryString = {
...
function processPendingLabResultRequest(patientUuid){
  return new Promise((resolve, reject) => {
    ...
    .then((result) => {
          if (result === false) {
              getCachedPendingLabResults(patientUuid).then((cachedResults) => {
                  let cachedEidResult = [];
