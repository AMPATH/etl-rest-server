...
export class FamilyTestingService {
  getPatientList = (params) => {
    return new Promise((resolve, reject) => {
      ...
      let where = '';
      let sql = 'SELECT ';
      const columns = `t1.*,tx.*, t8.status, tx.encounter_datetime as last_date_elicited,tx.gender as index_gender,
      case 
      ...
      if (params.elicited_clients == 0) {
        ...
        sql = this.reviwedWithChildren(params, sql, from);
      } else if (params.elicited_clients == 9) {
        //Reviewed with sexual partners
        sql = this.reviwedWithSexualPartners(params, sql, from);
  ...
  allReviewedClientsQuery(params, sql, from) {
    let w = ` WHERE t8.location_id in (${params.locations})  AND (DATE(t8.endDate) = DATE(t9.last_released_month)) `;
    if (params.start_date != null && params.end_date != null) {
      w =
        w +
        `  AND ((DATE(t1.date_elicited) BETWEEN date('${params.start_date}') and date('${params.end_date}'))
    ...
    sql =
      sql +
      ' tx.*, t8.status, extract(year from (from_days(datediff(now(),tx.birthdate)))) as age, tx.gender as index_gender, 1 as hideContactColumns ' +
      from +
      w +
  ...
  screenedNotReviewed(params, sql, from) {
    let w = ` WHERE t8.location_id in (${params.locations})  
  ...
  reviwedWithChildren(params, sql, from) {
    let w = ` WHERE t8.location_id in (${params.locations})  
  ...
  reviwedWithSexualPartners(params, sql, from) {
    let w = ` WHERE t8.location_id in (${params.locations})  
              AND (IF(obs_group_id IS NOT NULL
